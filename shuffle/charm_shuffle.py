...
g = open(path + indexBase + '.csv')
# no header to skip

for i in range(nUnderway):
    line = g.readline()
    index.append(line.split(',')[8:])
    # This clears away that linefeed hanging on to the last element of the row in index[i]
    index[i][nCharms - 1] = index[i][nCharms - 1].rstrip('\n')

g.close()

# Now I have to convert index[][] into integer values
for i in range(nUnderway):
    for j in range(nCharms):
        index[i][j] = int(index[i][j])

print '\n    ok we are underway with an index.\n\n'
...
# The output file
f = open(path + outputBase + '.csv', 'w')


# Let's loop over the nUnderway rows
for i in range(nUnderway):

    # Let's append to udata[i], write it, and delete that memory
    ...
    # udata[] is a list of lists.
    # len(udata[i]) tells how many elements in the list which is the ith element of udata[] 
    rowLen = len(udata[i])

    # Let's print everything in the i'th list to the output file separated by commas
