...
def Entrance():
    ...
    while stuckEntrance:
        ...
        a = WhatDoYouDo(['light my torch with my last match',\
                         'run back down the path I came here on',\
                         'knock on the front door',\
                         'try to open the front door',\
                         'wait for something to happen'])

        if 'match' in a or ('light' in a and 'torch' in a):
        ...
        elif len(a) == 1 and 'look' in a: time.sleep(0.001)
        else:
            sorry()
    
    # To reach here we know: stuckEntrance == False
    Foyer()
    return
...
def Foyer():
    ...
    while stuckFoyer:
        if foyerSpiderAsleep:
            ...
            a = WhatDoYouDo(['take a nap', 'go up the stairs', 'go back outside', 'go through the doors', 'tickle the spider'])
            if 'nap' in a:
            ...
            a = WhatDoYouDo(['Pull out the little sword and attack the spider',\
...
def Library():
    ...
    p('There are stairs leading down, a window, Kilroy                  ')
    a = WhatDoYouDo([\
                'read all the books',\
                'search the shelves for interesting clues',\
                'do my best Hermione impersonation',\
                'light a book on fire',\
